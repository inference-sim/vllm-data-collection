...
def test_bento():
    ...
    errors = {"ttft MAPE": [], "itl MAPE": [], "e2e MAPE": []}
    ...
    for idx, row in enumerate(df.itertuples()):
        ...
        if model_exists:
            ...
            try:
                ...
                e2e = get_metric("E2E", r"E2E:\s*([\d\.]+)", result.stdout)

                print(f"TTFT: {ttft}")
                print(f"ITL: {itl}")
                print(f"E2E: {e2e}")
                errors["ttft MAPE"].append(abs(ttft - row.ttft_mean)/row.ttft_mean * 100)
                errors["itl MAPE"].append(abs(itl - row.itl_mean)/row.itl_mean * 100)
                errors["e2e MAPE"].append(abs(e2e - row.e2e_mean)/row.e2e_mean * 100)
